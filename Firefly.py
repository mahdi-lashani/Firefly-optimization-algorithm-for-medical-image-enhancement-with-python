...
# Firefly parameters
Beta   = 2   # attraction coefficient base value
Lambda = 1   # Lihgt absorbtion coefficient
Alpha  = 0.2 # Mutation coefficient
E      = 0   # Uniformly random number
...
best_random_image_Firefly = transform_image(image_1, X_best[0], X_best[1], X_best[2], X_best[3], M_1)
best_random_image_Firefly = np.clip(best_random_image_Firefly, 0, 255).astype(np.uint8)
cv2.imwrite("E:\\ali_project\\best_random_image_Firefly.jpg", best_random_image_Firefly)
cv2.imshow ("E:\\ali_project\\best_random_image_Firefly.jpg", best_random_image_Firefly)
...
while FCR < FCR_max:
    ...
    for p in range(POP):
        for k in range(POP):
            r_p_k = (np.linalg.norm(np.subtract(X[p] , X[k]))) / (np.linalg.norm(np.subtract(L, U)))
            X[p] = X[p] + Beta * np.dot(math.exp(-Lambda * (r_p_k ** 2)), np.subtract(X[p], X[k]))
...
if X_best[2] > 0.96:
    result_image_Firefly = transform_image(image_1, X_best[0], X_best[1], X_best[2], X_best[3], M_1 * 1.9)
elif X_best[2] < 0.96 and X_best > 0.93:
    result_image_Firefly = transform_image(image_1, X_best[0], X_best[1], X_best[2], X_best[3], M_1 * 1.2)
elif X_best[2] < 0.93 and X_best > 0.89:
    result_image_Firefly = transform_image(image_1, X_best[0], X_best[1], X_best[2], X_best[3], M_1 * 0.9)
else :
    result_image_Firefly = transform_image(image_1, X_best[0], X_best[1], X_best[2], X_best[3], M_1)#/25.5

result_image_Firefly = np.clip(result_image_Firefly, 0, 255).astype(np.uint8)
cv2.imwrite("E:\\ali_project\\result_image_Firefly.jpg", result_image_Firefly)
cv2.imshow ("E:\\ali_project\\result_image_Firefly.jpg", result_image_Firefly)
